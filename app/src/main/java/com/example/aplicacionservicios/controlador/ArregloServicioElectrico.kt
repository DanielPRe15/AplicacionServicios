...
class ArregloServicioElectrico {

    fun adicionar(bean: ServicioElectrico): Int {
        var salida = -1
        ...
        row.put("cod_tiposervicioelec", bean.codigoTipo)
        ...
        salida = cn.insert("tb_servicioelec", "cod_servicioelec", row).toInt()
        return salida
    }

    fun listado():ArrayList<ServicioElectrico>{
        var data=ArrayList<ServicioElectrico>()
        //abrir el acceso a la base de datos en modo lectura
        var cn: SQLiteDatabase = appConfig.BD.readableDatabase
        //sentencia SQL
        var SQL="select * from tb_servicioelec"
        ...
        while(rs.moveToNext()) {
            ...
            val bean = ServicioElectrico(codigoServicio, codigoServi, codigoTipo,
